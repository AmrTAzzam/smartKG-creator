...
void help()
{
	...
	cout
		...
		<< endl;
	cout
			<< "\t-S\t\t\t Group with a minimum number of subjects (0.01% by default). We don't apply this filter by default, but it is recommended for very unstructure datasets (e.g. dbpedia)"
			<< endl;
	cout
...
void computeStatsPredicate(){
	...
	for (size_t i=0;i<hdt_file1->getDictionary()->getNpredicates();i++){
		...
		//if (verbose)
//		 cout<<"Estimation predicate,"<<(i+1)<<","<<hdt_file1->getDictionary()->idToString(i+1,PREDICATE)<<","<<it->estimatedNumResults()<<endl;
		//if (verbose) 
		//	cout<<"Percentage predicate,"<<(i+1)<<","<<StatsTriplesperPredicate[i]<<endl;
	}
//	exit(1);
}
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, "hve:o:s:iuGdm:ScP:L:H:q")) != -1)
	{
		switch (c)
		{
		case 'h':
		...
		case 'c':
			...
			break;
		case 'P':
			PERCENTAGE_SUBJECT_GROUPS  = atof(optarg);
			break;
		case 'L':
			PERCENTAGE_INFREQUENT_PREDICATES  = atof(optarg);
			break;
		case 'H':
			PERCENTAGE_CUT_PREDICATES  = atof(optarg);
			break;
		default:
	...
	double skip=0.1;
	// Uncomment to test different infrequent predicates
	//for (PERCENTAGE_INFREQUENT_PREDICATES=0.3;PERCENTAGE_INFREQUENT_PREDICATES<1;PERCENTAGE_INFREQUENT_PREDICATES+=skip){
	for (PERCENTAGE_CUT_PREDICATES=1;PERCENTAGE_CUT_PREDICATES<=1;PERCENTAGE_CUT_PREDICATES+=skip){
		//if (PERCENTAGE_INFREQUENT_PREDICATES==0.1){
	//		skip=0.1;
	//	}	
		...
		prevPredicate = 0;
		family = "";
//		map<unsigned int, vector<unsigned int>> familiesPerPredicate;
...
//vector<vector<unsigned int>> predicatesPerFamily; // the predicates of each family
		...
		while (it1->hasNext())
		{
			...
			if (numTriples%10000000==0){
				cerr<<"   > Processing "<<numTriples<<" triples"<<endl;
			}
		...
		delete it1;
		cout<<"Total triples: "<<numTriples<<endl;
...
//		}
...
} // END of loop to test different parameters
		cout<<"tau_l min pred:"<<PERCENTAGE_INFREQUENT_PREDICATES<<endl;
		cout<<"tau_h max pred:"<<PERCENTAGE_CUT_PREDICATES<<endl;
		
		//if (!groupFilterSubjects){
		if (!groupFilterSubjects){
			cout<<"Number of families:"<<predicatesPerFamily.size()<<endl;
			cout<<"Subject cut in %: Not applied"<<endl;
		}
		else { // if we restrict the families with a min number of subjects, then first identify these families
			cout<<"subj Number of families:"<<predicatesPerFamily.size()<<endl;
			cout<<"   - Get the families with at least "<<MIN_SUBJECT_GROUP<<" subjects"<<endl;

			//first check the families with more predicates
			for (int i=0;i<predicatesPerFamily.size();i++){
				if (subjectsPerFamilies[i+1].size()>=MIN_SUBJECT_GROUP){ //0.01% of number of subjects
			...
			cout<<"Subject cut in %:"<<PERCENTAGE_SUBJECT_GROUPS<<endl;
			cout<<"Number of families:"<<familiesLargerSubjects.size()<<endl;
			cout<<"  - With Subject cut, these families are present in >= "<<MIN_SUBJECT_GROUP<<" ("<<PERCENTAGE_SUBJECT_GROUPS<<" percent of the subjects)"<<endl<<endl;
		...
		if (quickEstimate) {
			cout<< "Quick estimate --> leaving" << endl ;
			exit(1);
		}
			
		cout<< "Full estimate..."<< endl;
		...
		if (groupPredicates){ //grouping activated
			...
			if (groupFilterSubjects){ // if we group restricting to a minimum number of covered subjects
				...
				for (int j=0;j<predicatesPerFamily.size();j++){
					...
					if ((j%1000)==0)
						cerr<<j<<" families processed"<<endl;
				}
	...
	//			for (int i=0;i<familiesLargerSubjects_fromMerge.size();i++)
		//			familiesLargerSubjects_total.push_back(familiesLargerSubjects_fromMerge[i]);
				...
				cerr<<endl<<"numFound intersects:"<<numFound<<" out of "<<predicatesPerFamily.size()<<" families"<<endl<<endl;
				cerr<<endl<<"new Families:"<<familiesLargerSubjects_fromMerge.size()<<endl<<endl;

				initial_size_without_grouping = predicatesPerFamily.size();
				// prepare auxiliary structures, familyToMap
				cerr<<"prepare auxiliary structures, familyToMap"<<endl;
				for (int i=0;i<initial_size_without_grouping;i++){
					predToMergeFam.clear();
					addPredicatesToMergeNoRecursive((i+1));
					if (i%1000==0)
						cerr<<"Completing the predicates of family:"<<i<<endl;
					}


			}
			else{ //normal grouping
				cerr<< " Initial groups: "<<initial_size_without_grouping<<endl;

				//grouping
				size_t num_new_groups = group(0,initial_size_without_grouping);

				cerr<< "Total potential new groups: "<<num_new_groups<<endl;
	...
	//				cout<<"current familyToMap[i+1] size:"<<familyToMap[i+1].size()<<endl;
			...
			cerr<<"do a first pass to compute the exact stats of the number of triples and decide if a family is not materialized"<<endl;
		...
		if (groupFilterSubjects){
			cout<<"Final number of families:"<<familiesLargerSubjects_total.size()<<endl;
	
		}else{
			cout<<"Final number of families:"<<predicatesPerFamily.size()<<endl;
		}
